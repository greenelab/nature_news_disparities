...
freq_idx = paste(class_m_ids_keep$Var1, class_m_ids_keep$Var2, sep="_")
class_m_ids = subset(class_m_ids, freq_idx %in% freq_idx)
...
class_c_word_freq = get_word_freq_per_class(class_c_ids, class_str = "class_c")
class_m_word_freq_ph = get_word_freq_per_class(subset(class_m_ids, address.country_code == "in"), class_str = "class_m_ph")


# merge the classes together
# only take words that are in both
per_class_word_freq = merge(data.table(class_c_word_freq), data.table(class_m_word_freq_ph), by="word")

# it should be more than 100 in at least one corpus
per_class_word_freq = subset(per_class_word_freq, 
                             class_c_count > 10 & class_m_ph_count > 10)

# now calculate the log odds ratio
per_class_word_freq$ratio = per_class_word_freq$class_c_count / 
                            per_class_word_freq$class_m_ph_count
