...
pipeline_1_dir = file.path(proj_dir,
...
for(curr_file in pipeline_1_files){
    ...
    # if there was something to read, but none important, skip
    ...
    article_ids = unique(article_ids)
    ...
    year_level_vec = c(curr_year, num_articles, file_name_type,
...
colnames(year_stats_df) = c("year", "num_articles", "article_type",
                            "num_duplicated_ids", "num_empty_files")
colnames(article_stats_df) = c("year", "article_ids", "num_words")

year_stats_df$year = as.numeric(year_stats_df$year)
year_stats_df$num_articles = as.numeric(year_stats_df$num_articles)
year_stats_df$num_empty_files = as.numeric(year_stats_df$num_empty_files)
year_stats_df$num_duplicated_ids = as.numeric(year_stats_df$num_duplicated_ids)

article_stats_df$year = as.numeric(article_stats_df$year)
article_stats_df$num_words = as.numeric(article_stats_df$num_words)
```
...
for(curr_dir in pipeline_2_dir){
    ...
    expected_num_files = year_stats_df$num_articles[
                                ...
                                year_stats_df$article_type == file_name_type]
    empty_files = year_stats_df$num_empty_files[
                                year_stats_df$year == file_name_year &
                                year_stats_df$article_type == file_name_type]

    # if there are no articles of this type, for this year, skip
    if(length(expected_num_files) == 0 | length(empty_files) == 0){
        next
    }
    ...
    text_df = data.frame(file_id=rep(NA,length(pipeline_2_files_body)),
                        ...
                        year=file_name_year,
                        file_type=rep(file_name_type,length(pipeline_2_files_body)))
    idx = 1
    for(curr_file in pipeline_2_files_body){
        in_text = read_lines(curr_file)
        text_df[idx,] = c(curr_file, in_text, file_name_year, file_type=file_name_type)
        idx = idx+1
    }
...
for(curr_dir in pipeline_3_dir){
    ...
    # if the folder is empty, continue
    if(length(pipeline_3_json_files) == 0){
        next
    }
...
# we will not analyze the guardian locations mentioned so we filter it out
