...
quote_prop_df = quote_prop_df[,colnames(springer_prop_df)]

    
# compare foreground and background
compare_df = rbind(springer_prop_df, quote_prop_df)
...
ggplot(quote_prop_df, aes(x=as.numeric(year), y=male_prop,
    ...
    geom_point() + geom_line() + theme_bw() + 
    xlab("Year of Article") + ylab("Male First Author Citation Percentage") +
    ggtitle("Comparison of Male First Author Quotes Over Time") + 
    scale_fill_brewer(palette="Set2") + ylim(c(0.5, 0.95)) +
    geom_hline(yintercept=0.5, color="red")
...
springer_prop_df = springer_prop_df[,colnames(compare_df)]

compare_df = rbind(compare_df, springer_prop_df)
compare_df = subset(compare_df, year > 2017 )
ggplot(compare_df, aes(x=as.numeric(year), y=male_prop,
                              fill=corpus, color=corpus)) +
    geom_point() + geom_line() + theme_bw() + 
