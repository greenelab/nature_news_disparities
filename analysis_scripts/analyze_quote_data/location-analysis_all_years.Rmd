...
```{r fig.align='center', echo=FALSE, warning=FALSE, message=F}
    ...
    un_region_df = unique(bm_full_loc_df[,c("is_benchmark", "year", 
                                            "est_un_region", "est_country")])
    ggplot(un_region_df, aes(x=is_benchmark, fill=est_un_region)) +
        geom_bar(position="fill") + facet_grid(~year) + theme_bw() + 
        xlab("Year of Article") + ylab("UN Region Proportions") +
        ggtitle("UN Region mentions over Time") + 
