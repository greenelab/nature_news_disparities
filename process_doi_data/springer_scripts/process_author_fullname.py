...
# Load model
model = load_model("%s/LSTM.h5" % args.model_path)
filename = args.model_path + "/LSTM_idx_dic.pkl"
idx_dic = pickle.load(open(filename, "rb"))
categories = pd.read_csv("%s/LSTM_categories.txt" % args.model_path, header=None)
