...
class NewsSpider(scrapy.Spider):
    ...
    def parse_article(self, response, year=None):
        ...
        # # get journalist names

        journalist_list = response.css('ul.c-article-author-list > li > a::text')


        journalist_list = [
            {'name': x.strip("\xa0")}
            for x in journalist_list.getall()
        ]

        parsed_as_year = "2016_or_later"

        # get the article body for matching later
        ...
        yield {
           ...
           "url": response.url,
           "year": year,
           "authors": journalist_list,
           "parsed_as_year": parsed_as_year,
           "body": article_body
