...
gg_gender = draw_confusion_matrix(confusion_matrix, "FEMALE", "MALE", "Gender prediction Conf. Matr.")
...
gg_loc_hist = ggplot(country_errors, aes(x=num_fp)) +
                geom_histogram() +
                theme_bw() + 
                xlab("Number of Articles with False Country Mention") +
                ggtitle("Number of Articles with False Country Mention in Benchmark") + 
...
ggsave(file.path(proj_dir, "/figure_notebooks/tmp_files/supp_fig1_tmp/gg_loc_line.pdf"),
       gg_loc_line, width = 10, height = 5, units = "in", device = "pdf")
...
bottom_image <- image_append(image_scale(c(gg_loc_hist, gg_loc_line), "x500"), stack = FALSE)
full_image <- image_append(image_scale(c(gg_gender, bottom_image), 3000), stack = TRUE)

print(full_image)

outfile = file.path(proj_dir,"/figure_notebooks/tmp_files/supp_fig1_tmp/supp_fig1.pdf")
