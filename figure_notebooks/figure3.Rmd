...
quote_name_df = read_name_origin_prob(name_pred_file, name_info_file)
#quote_name_df$name_origin[quote_name_df$name_origin == "Jewish"] = "Hebrew"
colnames(quote_name_df)[which(colnames(quote_name_df) == "Jewish")] = "Hebrew"
...
mentioned_name_df = read_name_origin_prob(name_pred_file, name_info_file)
#mentioned_name_df$name_origin[mentioned_name_df$name_origin == "Jewish"] = "Hebrew"
colnames(mentioned_name_df)[which(colnames(mentioned_name_df) == "Jewish")] = "Hebrew"
...
cite_name_df = read_name_origin_prob(name_pred_file, name_info_file)
#cite_name_df$name_origin[cite_name_df$name_origin == "Jewish"] = "Hebrew"
colnames(cite_name_df)[which(colnames(cite_name_df) == "Jewish")] = "Hebrew"
...
col_ids = c("author", "year", "corpus", "doi", "African", "CelticEnglish", 
            "EastAsian", "European", "Greek", "Hispanic", 
            "Hebrew", "ArabTurkPers", "Nordic", "SouthAsian")
...
# helper method for calling the bootstrap, using probabilities
get_subboot_prob <- function(origin_id, curr_corpus, in_df, bootstrap_col_id="doi"){
    bootstrap_res = compute_bootstrap_location_prob(subset(in_df, 
                                                      corpus==curr_corpus), 
                                              year_col_id = "year", 
                                              article_col_id = bootstrap_col_id, 
                                              country_col_id = origin_id,
                                              conf_int = 0.95)
    bootstrap_df = bootstrap_res$quantile_res
    bootstrap_raw = bootstrap_res$boot_res
...
call_get_subboot_prob <- function(name_df, curr_corpus){
    
    # get the bootstrapped CI for each source data type
    origin_df = NA
    origin_raw = NA
    for(curr_origin in name_origin_ids){
        print(curr_origin)
        res = get_subboot_prob(curr_origin, 
                          curr_corpus=curr_corpus,
                          name_df)
        origin_df = rbind(origin_df, res[[1]])
        origin_raw = rbind(origin_raw, res[[2]])
...
if(RERUN_BOOTSTRAP){
    ...
    raw_bootstrap_df = raw_bootstrap_df[which(raw_bootstrap_df$year < 2021), ]
    ...
    all_bootstrap_df = all_bootstrap_df[which(all_bootstrap_df$year < 2021), ]
...
get_FC_summary <- function(raw_bootstrap_df, curr_corpus_fg, curr_corpus_bg,
    ...
    for(curr_name_origin in name_origins){
        ...
        common_years = intersect(curr_fg_df$year, curr_bg_df$year)
        curr_bg_df = curr_bg_df[which(curr_bg_df$year %in% common_years), ]
        curr_fg_df = curr_fg_df[which(curr_fg_df$year %in% common_years), ]
...
citation_j_overview_gg2 = ggplot(citation_j_origin_df2, aes(x=as.numeric(year), y=mean,
                          ...
                          fill=name_origin, label=label)) +
    geom_point() + geom_ribbon(alpha=0.5) + geom_line(alpha=0.5) +
    theme_bw() + geom_text_repel() + xlim(c(2005,2020))  +
    xlab("Year of Article") + ylab("Proportion of Journalist Citations") +
